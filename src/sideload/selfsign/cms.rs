...
pub fn sign_code_directory(
    ...
    println!("[cms] Build signed attributes...");
    let signed_attrs_concat = build_all_signed_attributes(code_directory)
        .context("Build signed attributes fail")?;

    // Signing input = DER SET wrapping all attrs
    let attrs_set_for_signing = encode_tagged(TAG_SET, &signed_attrs_concat);

    println!("[cms] RSA sign {} bytes...", attrs_set_for_signing.len());
    let signature = rsa_sign_sha256(&identity.key_pem, &attrs_set_for_signing)
        .context("RSA sign fail")?;

    println!("[cms] Build SignerInfo...");
    let signer_info = build_signer_info(identity, &signed_attrs_concat, &signature)
        .context("Build SignerInfo fail")?;

    println!("[cms] Build SignedData...");
    let signed_data = build_signed_data(identity, &signer_info)
        .context("Build SignedData fail")?;
    ...
    println!("[cms] OK — {} bytes", content_info.len());
    Ok(content_info)
}
...
fn build_signer_info(
    ...
    inner.extend(encode_tagged(TAG_INTEGER, &[0x01]));       // version = 1
    inner.extend(issuer_serial);                              // sid
    inner.extend(digest_alg);                                 // digestAlgorithm
    inner.extend(encode_tagged(TAG_CTX_0, signed_attrs_concat)); // signedAttrs
    inner.extend(sig_alg);                                    // signatureAlgorithm
    inner.extend(encode_tagged(TAG_OCTET_STRING, signature)); // signature
...
fn build_signed_data(
    ...
    // [0] certificates
    let certs = build_certificates(identity)?;
...
mod tests {
    ...
    #[test]
    fn test_encode_oid_sha256() {
        let oid = encode_oid(OID_SHA256);
        assert_eq!(oid[0], TAG_OID);
        assert_eq!(oid[1], 9);
        assert_eq!(oid[2], 0x60);  // 2*40+16
    }
}

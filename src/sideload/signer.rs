...
pub fn sign_app(
    ...
    // Bundle dir đã extract
    let bundle_dir = app.bundle.bundle_dir.clone();

    // Output IPA
    let output_ipa = std::env::temp_dir()
        .join("fortiva_signed.ipa");
    ...
    // Copy IPA signed ngược về bundle dir (để install flow dùng)
    println!("[sign] Extract signed IPA back to bundle dir...");
    extract_ipa_to_dir(&output_ipa, &bundle_dir.parent().unwrap())?;

    Ok(())
}

/// Extract IPA vào thư mục đích (giữ cấu trúc Payload/).
fn extract_ipa_to_dir(ipa_path: &std::path::Path, dest: &std::path::Path) -> Result<()> {
    use std::io::Read;
